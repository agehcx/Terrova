...
GeoProof handles financial transactions and sensitive user data. Security is non-negotiable.
...
NEXT_PUBLIC_GEOPROOF_PROGRAM_ID=
```
    ...
    return [
      {
        source: '/api/:path*',
        headers: [
          { key: 'Access-Control-Allow-Origin', value: 'https://geoproof.io' },
...
async function validateTransaction(tx: Transaction): Promise<boolean> {
  ...
  for (const ix of tx.instructions) {
    if (!ix.programId.equals(GEOPROOF_PROGRAM_ID)) {
...
#[account(
    mut,
    constraint = authority.key() == node.authority @ GeoProofError::Unauthorized
)]
...
let new_balance = node.stake
    .checked_add(amount)
    .ok_or(GeoProofError::ArithmeticOverflow)?;

// Validate account ownership
#[account(
    constraint = node.owner == program_id @ GeoProofError::InvalidOwner
)]
...
Security issues: security@geoproof.io
