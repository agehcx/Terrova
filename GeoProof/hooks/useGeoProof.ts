...
import { GeoProofClient } from '@/lib/geoproof/client';

export function useGeoProof() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const geoProofClient = new GeoProofClient(connection, wallet, programId);
      setClient(geoProofClient);
      setIsInitialized(true);
      setError(null);
    } catch (err) {
      console.error('[useGeoProof] Initialization error:', err);
      setError(err instanceof Error ? err.message : 'Failed to initialize GeoProof client');
      setIsInitialized(false);
    }
  ...
  const registerNode = useCallback(
    async (name: string, latitude: number, longitude: number, region: string) => {
      if (!client) {
        setError('GeoProof client not initialized');
