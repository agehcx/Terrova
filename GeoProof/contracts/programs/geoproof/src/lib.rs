...
declare_id!("GEOprooF111111111111111111111111111111111111");

#[program]
pub mod geoproof {
    use super::*;

    /// Initialize the GeoProof protocol with admin settings
    ...
    pub fn register_node(
        ...
        require!(
            ...
            GeoProofError::InsufficientStake
        );
    ...
    pub fn submit_evidence(
        ...
        require!(node.status == NodeStatus::Active, GeoProofError::NodeNotActive);
        require!(
            is_in_range(&node.location, &request.location, request.radius_km),
            GeoProofError::NodeOutOfRange
        );
    ...
    pub fn finalize_verification(ctx: Context<FinalizeVerification>) -> Result<()> {
        ...
        require!(
            ...
            GeoProofError::InsufficientEvidence
        );
...
pub struct SubmitEvidence<'info> {
    ...
    #[account(
        mut,
        has_one = owner @ GeoProofError::Unauthorized
    )]
    pub node: Account<'info, Node>,
    #[account(mut)]
